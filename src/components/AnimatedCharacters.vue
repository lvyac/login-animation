    ...
    <div ref="purpleRef" class="char purple" :style="purpleStyle">
      <div class="eyes" :style="purpleEyesStyle">
        <EyeBall v-for="i in 2" :key="'p'+i" :size="18" :pupil-size="7" :max-distance="5"
          eye-color="white" pupil-color="#2D2D2D" :is-blinking="isPurpleBlinking || closingEyes"
    ...
    <div ref="blackRef" class="char black" :style="blackStyle">
      <div class="eyes" :style="blackEyesStyle">
        <EyeBall v-for="i in 2" :key="'b'+i" :size="16" :pupil-size="6" :max-distance="4"
          eye-color="white" pupil-color="#2D2D2D" :is-blinking="isBlackBlinking || closingEyes"
...
const props = withDefaults(defineProps<{
  ...
  isTyping?: boolean
  /** Whether the user is currently typing in password field */
...
const closingEyes = computed(() => props.isTypingPassword)
...
const purpleEyesStyle = computed(() => ({
  left: hiding.value ? `${20 + shakeHeadOffset.value}px` : isLookingAtEachOther.value ? `${55 + shakeHeadOffset.value}px` : `${45 + purplePos.faceX + shakeHeadOffset.value}px`,
...
const blackEyesStyle = computed(() => ({
  left: hiding.value ? `${10 + shakeHeadOffset.value}px` : isLookingAtEachOther.value ? `${32 + shakeHeadOffset.value}px` : `${26 + blackPos.faceX + shakeHeadOffset.value}px`,
...
const orangeEyesStyle = computed(() => ({
  left: hiding.value ? `${50 + shakeHeadOffset.value}px` : `${82 + orangePos.faceX + shakeHeadOffset.value}px`,
...
const yellowEyesStyle = computed(() => ({
  left: hiding.value ? `${20 + shakeHeadOffset.value}px` : `${52 + yellowPos.faceX + shakeHeadOffset.value}px`,
  top: hiding.value ? '35px' : `${40 + yellowPos.faceY}px`,
  gap: '24px',
}))
const yellowMouthStyle = computed(() => ({
  left: hiding.value ? `${10 + shakeHeadOffset.value}px` : `${40 + yellowPos.faceX + shakeHeadOffset.value}px`,
  top: hiding.value ? '88px' : `${88 + yellowPos.faceY}px`,
}))

// Shake head animation for login failure
function shakeHead() {
  const sequence = [-15, 15, -12, 12, -8, 8, 0]
  let i = 0
  const interval = setInterval(() => {
    shakeHeadOffset.value = sequence[i]
    i++
...
.eyes { position: absolute; display: flex; transition: all 0.1s ease-out; }
.mouth { position: absolute; width: 80px; height: 4px; background: #2D2D2D; border-radius: 4px; transition: all 0.1s ease-out; }
</style>
